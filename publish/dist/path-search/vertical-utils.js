...
function findVerticals(startCoords, endCoords, accessibility = false) {
    const ret = [];
    let midFloorCoords = { ...startCoords };
    let reachedFloor = endCoords.floor;
    let reachedFloorIdx = utils_1.floorNums.findIndex(n => n === endCoords.floor);
    // let dir = startCoords.floor < endCoords.floor ? -1 : 1;
    let dir = Math.sign(startCoords.floor - endCoords.floor);
    if (reachedFloor % 1 !== 0 && startCoords.floor % 1 !== 0 && startCoords.floor % 1 !== reachedFloor % 1) {
        dir = 1;
        reachedFloor = 5;
        reachedFloor = findNearestCommonReachedFloor(startCoords.floor, endCoords.floor);
    }
    while (reachedFloor !== startCoords.floor) {
        let vertical = findCheapestVertical(midFloorCoords, { x: endCoords.x, y: endCoords.y, floor: reachedFloor }, accessibility);
        if (vertical) {
            ...
            reachedFloorIdx = (reachedFloorIdx += dir) % utils_1.floorNums.length;
            reachedFloor = utils_1.floorNums[reachedFloorIdx];
        }
    }
...
function findCheapestVertical(startCoords, endCoords, accessibility = false) {
    const verticals = vertical_paths_1.verticalPaths;
    let paths = verticals.filter(v => v[4].findIndex((f) => f === endCoords.floor) !== -1 &&
        v[4].findIndex((f) => f === startCoords.floor) !== -1 &&
        utils_1.isAccessible(v.type) === accessibility);
    if (paths.length > 0) {
        let closest;
        ...
        paths.forEach(p => {
            ...
            // + p.cost * Math.abs(startCoords.floor - endFloor);
            if (dist < minDistance) {
                closest = p;
                minDistance = dist;
            }
...
function findNearestCommonReachedFloor(startFloor, endFloor) {
    let reachedFloor = startFloor;
    let minDist = Number.MAX_VALUE;
    utils_1.floorNums.forEach(n => {
        if (doesFloorReach(n, startFloor, endFloor)) {
...
function doesFloorReach(floor, ...endFloors) {
    for (let i = 0; i < vertical_paths_1.verticalPaths.length; i++) {
        const p = vertical_paths_1.verticalPaths[i];
...
//# sourceMappingURL=vertical-utils.js.map